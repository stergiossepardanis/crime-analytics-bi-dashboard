...
SELECT
    ...
    dv.vict_id,
    dw.weapon_id
FROM dbo.crime_import AS ci
JOIN dbo.dimArea AS da ON
       ci.area = da.area
   AND ci.area_name = da.area_name
   AND ci.rpt_dist_no = da.rpt_dist_no
   AND ci.location = da.location
   AND ISNULL(ci.cross_street, '') = ISNULL(da.cross_street, '')
   AND ci.lat = da.lat
   AND ci.lon = da.lon
JOIN dbo.dimCrime AS dc ON
       ci.part_1_2 = dc.part_1_2
   AND ci.crm_cd = dc.crm_cd
   AND ci.crm_cd_desc = dc.crm_cd_desc
   AND ISNULL(ci.mocodes, '') = ISNULL(dc.mocodes, '')
   AND ISNULL(ci.crm_cd_1, 0) = ISNULL(dc.crm_cd_1, 0)
   AND ISNULL(ci.crm_cd_2, 0) = ISNULL(dc.crm_cd_2, 0)
   AND ISNULL(ci.crm_cd_3, 0) = ISNULL(dc.crm_cd_3, 0)
   AND ISNULL(ci.crm_cd_4, 0) = ISNULL(dc.crm_cd_4, 0)
   AND ci.status = dc.status
   AND ci.status_desc = dc.status_desc
JOIN dbo.dimPremise AS dp ON
       ISNULL(ci.premis_cd, 0) = ISNULL(dp.premis_cd, 0)
   AND ci.premis_desc = dp.premis_desc
JOIN dbo.dimVictim AS dv ON
       ci.vict_age = dv.vict_age
   AND ci.vict_sex = dv.vict_sex
   AND ci.vict_descent = dv.vict_descent
JOIN dbo.dimWeapon AS dw ON
       ISNULL(ci.weapon_used_cd, 0) = ISNULL(dw.weapon_used_cd, 0)
   AND ISNULL(ci.weapon_desc, '') = ISNULL(dw.weapon_desc, '')
WHERE NOT EXISTS (
    SELECT 1
    FROM dbo.factCrime AS fc
    WHERE fc.dr_no = ci.dr_no
);
